...
<section class="rpd-lpp-comparison rpd-lpp-section">
	<div class="rpd-container rpd-lpp-comparison__grid">
		<div>
			...
			<h2><?php echo wp_kses_post( rocketpd_lpp_get_field( 'rpd_lpp_comparison_headline', __( 'Start with LaunchPad. <br>Expand with LaunchPad<span>+</span>.', 'rocketpd' ) ) ); ?></h2>
			...
			<a class="rpd-btn rpd-btn--outline-purple" href="<?php echo esc_url( rocketpd_lpp_get_field( 'rpd_lpp_comparison_cta_url', home_url( '/launchpad/' ) ) ); ?>"><?php echo esc_html( rocketpd_lpp_get_field( 'rpd_lpp_comparison_cta_label', __( 'Explore LaunchPad', 'rocketpd' ) ) ); ?> <?php rocketpd_lpp_icon( 'arrow' ); ?></a>
		</div>
		<div class="rpd-lpp-compare-shell">
			<table class="rpd-lpp-compare-table">
					...
					<?php foreach ( $rows as $row ) : ?>
						...
						<?php if ( $capability ) : ?>
							<tr>
								...
								<td>
									<span class="<?php echo esc_attr( $lp ? 'is-included' : 'is-missing' ); ?>" aria-hidden="true"><?php rocketpd_lpp_icon( $lp ? 'check' : 'x' ); ?></span>
									<span class="rpd-visually-hidden"><?php echo esc_html( $lp ? __( 'Included', 'rocketpd' ) : __( 'Not included', 'rocketpd' ) ); ?></span>
								</td>
								<td>
									<span class="<?php echo esc_attr( $lpp ? 'is-included' : 'is-missing' ); ?>" aria-hidden="true"><?php rocketpd_lpp_icon( $lpp ? 'check' : 'x' ); ?></span>
				...
				<tfoot>
					<tr><td></td><td><?php esc_html_e( "For districts that want immediate access to RocketPD's library", 'rocketpd' ); ?></td><td><?php esc_html_e( 'For districts ready to centralize their entire PL environment', 'rocketpd' ); ?></td></tr>
				</tfoot>
			</table>
		</div>
	</div>
