...
$nav_cta_label       = rocketpd_get_option( 'rpd_primary_nav_cta_label', $is_contact_template ? __( 'Join the Community', 'rocketpd' ) : '' );
$nav_cta_url         = rocketpd_get_option( 'rpd_primary_nav_cta_url', $is_contact_template ? home_url( '/' ) : '' );
$login_label         = rocketpd_get_option( 'rpd_login_label', $is_contact_template ? __( 'Login', 'rocketpd' ) : '' );
$login_url           = rocketpd_get_option( 'rpd_login_url', $is_contact_template ? home_url( '/login/' ) : '' );
$has_header_actions = ( $nav_cta_label && $nav_cta_url ) || ( $login_label && $login_url );
$contact_nav_links  = array(
	array(
...
<header class="rpd-site-header">
	<div class="rpd-container rpd-site-header__inner">
		...
		<nav class="rpd-site-header__nav" aria-label="<?php esc_attr_e( 'Primary navigation', 'rocketpd' ); ?>">
			<?php if ( $is_contact_template ) : ?>
				<ul class="rpd-menu rpd-menu--primary">
					<?php foreach ( $contact_nav_links as $nav_link ) : ?>
						<li>
	...
	get_template_part(
		...
		array(
			...
			'nav_links'          => $is_contact_template ? $contact_nav_links : array(),
		)
	);
	?>
</header>
